...
sub run {
    ...
    assert_script_run "ipa service-add testservice/$hostname";
    assert_script_run "ipa-getkeytab -s $hostname -p testservice/$hostname";
    validate_script_output 'klist -k /tmp/testservice.keytab', sub { $_ =~ m/testservice\/$hostname/ };
    # This is commented for now. We need a while loop that watches for ipa-getcert list -r to become empty.
    #assert_script_run "ipa-getcert request -K testservice/$hostname -D $hostname -f /etc/pki/tls/certs/testservice.pki -k /etc/pki/tls/private/testservice.key";
